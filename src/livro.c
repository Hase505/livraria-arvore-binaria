...
int imprimir_dados(FILE* arquivo, size_t codigo) {
        ...
        if (status == SUCESSO) {
                printf(
                    "Codigo: %zu\nTitulo: %s\nAutor: %s\nEditora: %s\nEdicao: %zu\nAno: "
                    "%zu\nExemplares: %zu\nPreco: %lf\n\n",
                    res.no->livro.codigo, res.no->livro.titulo, res.no->livro.autor, res.no->livro.editora,
                    res.no->livro.edicao, res.no->livro.ano, res.no->livro.exemplares, res.no->livro.preco);
                free(res.no);
                free(res.pai);
                return SUCESSO;
        }
